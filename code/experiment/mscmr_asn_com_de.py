        ...
        with torch.no_grad():
            for img_c0, img_t2, img_de, lab_c0, lab_t2, lab_de, c0_path,t2_path,de_path in self.val_loader:
                ...
                if self.args.save_imgs==True:
                    ...
                    self.save_img_with_contorusV2(ori_img['de'],
                                                reindex_label_array_by_dict(ori_lab['de'],{1:[1,200,1220,2221]}),
                                                output + "_contours", self.renamepath(de_path, 'gt_con_img') + ".png",colorDe)
    ...
    def gen_feat_map(self):
        ...
        with torch.no_grad():
            for img_c0, img_t2, img_de, lab_c0, lab_t2, lab_de, c0_path,t2_path,de_path in self.val_loader:
                ...
                # train
                skip_c0,skip_t2,theta_c0,theta_t2 = self.model(img[Modality.c0],img[Modality.t2], img[Modality.de],True)
                warp_skip_c0=[]
                for i in skip_c0:
                    warp_skip_c0.append(self.model.warp(i,theta_c0))
                warp_skip_t2=[]
                for i in skip_t2:
                    warp_skip_t2.append(self.model.warp(i,theta_t2))
