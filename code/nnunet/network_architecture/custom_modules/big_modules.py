...
class PixelEncoder(nn.Module):
    def __init__(self, pix_feat_dim: int):
        ...
        self.proj1 = nn.Conv2d(256, pix_feat_dim, kernel_size=1)
        self.proj2 = nn.Conv2d(256, pix_feat_dim, kernel_size=1)

    def forward(self, x) -> (Tuple[torch.Tensor, torch.Tensor, torch.Tensor], torch.Tensor):
        x = self.conv1(x)   # ours:[bs, 16, dim/2, dim/2]
        x = self.bn1(x)
        x = self.relu(x)  # 1/2, 64 --> ours:16
        x = self.maxpool(x)  # 1/4, 64 --> 16
        f4 = self.res2(x)  # 1/4, 256 --> 64
        f8 = self.layer2(f4)  # 1/8, 512 --> 128
        f16 = self.layer3(f8)  # 1/16, 1024 --> 256
...
class MaskEncoder(nn.Module):
    def __init__(self, pix_feat_dim: int, value_dim: int):
        ...
        network = resnet18(pretrained=False, extra_dim=1)
        self.conv1 = network.conv1
        self.bn1 = network.bn1
        self.relu = network.relu  # 1/2, 64
        self.maxpool = network.maxpool
        ...
        self.fuser = GroupFeatureFusionBlock(pix_feat_dim, 64, value_dim, value_dim)    # feature fusion

        self.sensory_update = SensoryDeepUpdater(value_dim, value_dim)      # hidden reinforcer
    ...
    def forward(self,
        ...
        for i in range(0, num_objects, chunk_size):
            ...
            g_chunk = self.conv1(g_chunk)       # [24,2,128,128] -> [24,16,64,64]
            g_chunk = self.bn1(g_chunk)  # 1/2, 64  -> 1/2, 16
            ...
            g_chunk = g_chunk.view(batch_size, actual_chunk_size, *g_chunk.shape[1:])       # [bs,4,64,1/16]
            g_chunk = self.fuser(ms_features[0], g_chunk)       # [bs,4,128,1/16,1/16]
...
class MaskDecoder(nn.Module):
    def __init__(self, val_dim: int):
        ...
        self.fuser = GroupFeatureFusionBlock(128, val_dim, val_dim, val_dim)

        self.sensory_compress = GConv2D(val_dim + 1, val_dim, kernel_size=1)
        self.sensory_update = SensoryUpdater([val_dim, 64, 64 + 1], 128, 128) #

        self.decoder_feat_proc = DecoderFeatureProcessor([128, 64], [val_dim, 64])
        self.up_16_8 = MaskUpsampleBlock(val_dim, 64)   #
        self.up_8_4 = MaskUpsampleBlock(64, 64) #

        self.pred = nn.Conv2d(64, 1, kernel_size=3, padding=1)  #

        self.sensory_linear_pred = LinearPredictor(val_dim, 128)
    ...
    def forward(
        ...
        *,
        ...
        for i in range(0, num_objects, chunk_size):
            ...
            p8 = self.up_16_8(decoder_features[0], p16)     # upsample 1/16->1/8
