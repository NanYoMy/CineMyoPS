...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.max_num_epochs = 1000  #TODO fix it back to 1000 after debugging
    ...
    def run_training(self,ifouput):
        if not torch.cuda.is_available():
            self.print_to_log_file("WARNING!!! You are attempting to run training on a CPU (torch.cuda.is_available() is False). This can be VERY slow!")

        _ = self.tr_gen.next()
        # testpath = '/media/Data/LBG/Program/outputs/nnunet/raw/nnUNet_raw_data/Task015_heart_WHS'
        # data_np = _['data'][0].numpy()
        # data_np = np.transpose(data_np, (1, 3, 2, 0))
        # image11 = sitk.GetImageFromArray(data_np)
        # image11.SetSpacing((1.0, 1.0, 1.0))
        # sitk.WriteImage(image11, testpath + '/test.nii.gz')
        #
        # target_np = _['target'][0][0][0].numpy()
        # target_np = np.transpose(target_np, (0, 2, 1))
        # target11 = sitk.GetImageFromArray(target_np)
        # target11.SetSpacing((1.0, 1.0, 1.0))
        # sitk.WriteImage(target11, testpath + '/target1.nii.gz')
        #
        # target_np = _['target'][1][0][0].numpy()
        # target_np = np.transpose(target_np, (0, 2, 1))
        # target11 = sitk.GetImageFromArray(target_np)
        # target11.SetSpacing((1.0, 1.0, 1.0))
        # sitk.WriteImage(target11, testpath + '/target2.nii.gz')
        _ = self.val_gen.next()
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.use_progress_bar:
                ...
                for batch in range(self.num_batches_per_epoch):
                    num_board = self.epoch * (self.num_batches_per_epoch + 1) + batch
                    if ifouput==False:
                        l = self.run_iteration(self.tr_gen, True)       #
                    else:
                        l = self.run_iteration(self.tr_gen, num_board=num_board, writer=writer, do_backprop=True,
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    if ifouput==False:
                        l = self.run_iteration(self.tr_gen, False, True)
                    else:
                        l = self.run_iteration(self.val_gen, num_board=None, writer=None, do_backprop=False,
            ...
            if self.epoch % 20 == 0 and self.epoch > 50:
                self.save_checkpoint(join(self.output_folder, f"model_{self.epoch}.model"))
