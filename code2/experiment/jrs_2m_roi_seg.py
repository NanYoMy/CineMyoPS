...
from experiment.exp_myops20_jrs_roi_seg import Experiment
from tools.dir import sort_glob, mk_or_cleardir
from tools.set_random_seed import worker_init_fn


class Experiment_Consistent(Experiment):
    ...
    def __init__(self, args):
        ...
        if args.net == "tps":
            from jrs_networks.jrs_2m_tps_seg_net import JRS2MROITpsSegNet as RSNet
        else:
        ...
        self.trainall_loader = DataLoader(
            DataSetLoader(args, type="trainall", augo=True, task='pathology', ret_path=False),
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=4,
            pin_memory=True,
    ...
    def validate_net(self):
        ...
        with torch.no_grad():
            for img_c0, img_t2, img_de, lab_c0, lab_t2, lab_de, c0_path,t2_path,de_path in self.val_loader:
                ...
                warp_mv_seg = self.model.warp(mv_seg, theta, mode='nearest')
                mv_seg=torch.argmax(mv_seg,dim=1,keepdim=True)


                fix_seg=torch.argmax(fix_seg,dim=1,keepdim=True)
                warp_mv_seg = torch.argmax(warp_mv_seg, dim=1, keepdim=True)

                self.save_torch_img_lab(self.args.output_dir, img[Modality.de], roi_lab_lv[Modality.de], de_path, 'de_ori')
                self.save_torch_img_lab(self.args.output_dir,img[Modality.c0],roi_lab_lv[Modality.c0],c0_path,'c0_ori')
                self.save_torch_img_lab(self.args.output_dir, warp_img[Modality.c0], warp_mv_seg, c0_path, 'c0_warp')
    ...
    # def update_lr(self,optimizer,ep):
    #     optimizer.param_groups[0]['lr'] = poly_lr(ep, self.args.epochs,self.args.lr , 0.9)
    ...
    def train_eval_net(self):
        ...
        for epoch in range(self.args.init_epochs, self.args.epochs + 1):
            ...
            for img_c0, img_t2, img_de, lab_c0, lab_t2, lab_de in self.train_loader:
                # load data
                img, lab, roi_lab_myo, roi_lab_lv = self.create_torch_tensor(img_c0, img_de, img_t2, lab_c0, lab_de,
                                                                             lab_t2)
                warp_img = {}
                # warp_roi_lab_myo = {}
                # warp_roi_lab_lv = {}
                ...
                mv_seg, fix_seg, theta = self.model(img[Modality.c0], img[Modality.de])
                # loss
                warp_seg_lv[Modality.c0] = self.model.warp(mv_seg, theta)
                # warp_roi_lab_lv[Modality.c0] = self.model.warp(roi_lab_lv[Modality.c0], theta)
                warp_img[Modality.c0] = self.model.warp(img[Modality.c0], theta)
                loss_reg_lv = regcrit(fix_seg, warp_seg_lv[Modality.c0])
                # loss_reg_lv = regcrit(roi_lab_lv[Modality.de], warp_roi_lab_lv[Modality.c0])
                loss_seg_fix = segcrit(fix_seg, roi_lab_lv[Modality.de])
                loss_seg_mv = segcrit(mv_seg, roi_lab_lv[Modality.c0])
                loss_all = (loss_seg_fix + loss_seg_mv) + self.args.weight * (loss_reg_lv)
                # statistic
                sts_loss = {}
                sts_loss["train/reg_myo"] = (loss_reg_lv.item())
